...
pub enum OutOfRange {
    ...
    /// The target array size doesn't match the source register/sram size
    ArraySizeMismatch,
}
...
impl Display for OutOfRange {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            ...
            Self::ArraySizeMismatch => write!(f, "array size out of range"),
        }
    }
}
...
pub trait RegisterData {
    /// Construct an instance of the register from a slice of bytes
    fn from_bytes(bytes: &Vec<u8>) -> Result<Self, OutOfRange>
    where
        Self: Sized;

    /// Construct a vector of bytes from the register
...
/// compiler.
pub trait RegisterAccess<AddrType, ValueType> {
    type Register;
    ...
    fn set<P: Platform<AddrType, ValueType>, F: FnOnce(&mut Self::Register)>(
        &self,
        platform: &P,
        f: F,
    ) -> Result<(), P::Error>;
...
/// register.
pub trait RegisterInstance<AddrType> {
